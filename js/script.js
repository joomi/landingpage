...
//	$().enableUndo({ redoCtrlChar : 'y', redoShiftReq : false });
	...
	$(".lp_canvas").on('dblclick','a[data-role=link]', function(){
		...
		$.ajax({
			url:'includes/lp_editor_panels/link.tpl.php?link='+linkx,
			success: function(data){
				$('#lp_controls').append(data);
			}
		});
	...
	$(".lp_canvas").on('dblclick','iframe', function(){
		...
		$.ajax({
			url:'includes/lp_editor_panels/video.tpl.php?width=' + parseInt(width) + '&height=' + parseInt(height) + '&src=' + encodeURI(src),
			success: function(data){
				$('#lp_controls').append(data);
			}
		});
